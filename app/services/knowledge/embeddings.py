"""Embeddings OpenAI para pgvector. El modelo sale solo de .env."""
...
async def embed_texts(texts: Sequence[str]) -> List[Optional[List[float]]]:
    ...
    runtime = await get_llm_runtime()
    api_key = (runtime.openai_api_key or "").strip()
    if not api_key:
    ...
    model = get_settings().openai_embedding_model or "text-embedding-3-small"
